...
#' @param DistCutOff Distance in meters to use as a cut off radius around the colony to use to split the trips
#' @param Dist2Colony quoted name of column in data that has the distance from each point to the colony
...
MakeTrip<-function(data,ID="File",DistCutOff=50,Dist2Colony="Dist2Colony"){
  ...
  for(j in 1:length(Birds)){
    ...
    BirdSub$InColony2<-lead(BirdSub$InColony)

    # Detect state change for "out" events else NA
    BirdSub$InColony4<-ifelse(BirdSub$InColony==0&BirdSub$InColony2==1,"Out",NA)

    # Detect state change for "In" events else "out" or NA
    BirdSub$ColonyMovement<-ifelse(BirdSub$InColony==1&BirdSub$InColony2==0,"In",BirdSub$InColony4)

    # Get indicies of out events
    Out<-grep("Out",x = BirdSub$ColonyMovement)

    # If there is an "in " event get the indicies on the in events
    if("In" %in% BirdSub$ColonyMovement) In<-grep("In",x = BirdSub$ColonyMovement) else In<-length(BirdSub$ColonyMovement)-1

    # if the first in comes after the first out that make first index an out
    ...
    # add 1 to all the indecies in "in"
    In<-In+1
